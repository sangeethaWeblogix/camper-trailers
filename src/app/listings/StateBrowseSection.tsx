...
export default function StateBrowseSection({ state, region, category, initialData }: Props) {
            ...
            <div className="lsd-browse__row1">
              {showStatePanel && (
                <div className="lsd-browse__panel">
                  <h3 className="lsd-browse__panel-title">{`Browse ${label} Camping Trailers by State`}</h3>
              ...
              {showRegionPanel && (
                <div className="lsd-browse__panel">
                  <h3 className="lsd-browse__panel-title">{`Browse ${label} Camping Trailers by Popular Region`}</h3>
                  <div className="lsd-browse__pills">
                    {regionPanel.map((r) => (
              ...
              {showMakePanel && (
                <div className="lsd-browse__filter-col">
                  <div className="lsd-browse__filter-head">
                    ...
                    <span className="lsd-browse__filter-title">{`Browse ${label} Camping Trailers by Popular Manufacturers`}</span>
                  </div>
              ...
              {showPricePanel && (
                <div className="lsd-browse__filter-col">
                  <div className="lsd-browse__filter-head">
                    ...
                    <span className="lsd-browse__filter-title">{`Browse ${label} Camping Trailers by Price`}</span>
                  </div>
                  <div className="lsd-browse__filter-links">
                    {pricePanel.map((l) => (
                      <a key={l.text} href={l.href} className="lsd-browse__filter-link">{l.text}</a>
  ...
  if (stateRegionMode) {
    ...
    const panels = [
      { icon: "/images/Budget.png",   title: `Browse Camping Trailers by Price in ${regionName}`,            links: bandPanel(basePath, PRICE_BANDS, priceCounts) },
      { icon: "/images/ATM.png",      title: `Browse Camping Trailers by Weight (ATM) in ${regionName}`,      links: bandPanel(basePath, ATM_BANDS, atmCounts) },
      { icon: "/images/Sleeping.png", title: `Browse Camping Trailers by Sleeping Capacity in ${regionName}`, links: bandPanel(basePath, SLEEP_BANDS, sleepCounts) },
    ];
        ...
        <div className="container">
          ...
          {(showMakePanel || showCategoryPanel) && (
            <div className="lsd-browse__row1">
              {showMakePanel && (
                <div className="lsd-browse__panel">
                  <h3 className="lsd-browse__panel-title">{`Browse Camping Trailers by Popular Manufacturers in ${regionName}`}</h3>
                  <div className="lsd-browse__pills">
              ...
              {showCategoryPanel && (
                <div className="lsd-browse__panel">
                  <h3 className="lsd-browse__panel-title">{`Browse Camping Trailers by Type in ${regionName}`}</h3>
                  <div className="lsd-browse__pills">
                    {categoryPanel.map((c) => (
  ...
  if (categoryStateMode) {
    ...
    const panels = [
      { icon: "/images/Budget.png",   title: `Browse ${label} Camping Trailers by Price in ${stateName}`,            links: bandPanel(basePath, PRICE_BANDS, priceCounts) },
      { icon: "/images/ATM.png",      title: `Browse ${label} Camping Trailers by Weight (ATM) in ${stateName}`,      links: bandPanel(basePath, ATM_BANDS, atmCounts) },
      { icon: "/images/Length.png",   title: `Browse ${label} Camping Trailers by Size (Length) in ${stateName}`,     links: bandPanel(basePath, LENGTH_BANDS, lengthCounts) },
      { icon: "/images/Sleeping.png", title: `Browse ${label} Camping Trailers by Sleeping Capacity in ${stateName}`, links: bandPanel(basePath, SLEEP_BANDS, sleepCounts) },
    ];
        ...
        <div className="container">
          ...
          {(showRegionPanel || showMakePanel) && (
            <div className="lsd-browse__row1">
              {showRegionPanel && (
                <div className="lsd-browse__panel">
                  <h3 className="lsd-browse__panel-title">{`Browse ${label} Camping Trailers by Region in ${stateName}`}</h3>
                  <div className="lsd-browse__pills">
              ...
              {showMakePanel && (
                <div className="lsd-browse__panel">
                  <h3 className="lsd-browse__panel-title">{`Browse ${label} Camping Trailers by Popular Manufacturers in ${stateName}`}</h3>
                  <div className={`lsd-browse__pills${makePanel.length > 12 ? " lsd-browse__pills--scroll" : ""}`}>
                    {makeCounts === null ? pillSkeleton() : makePanel.map((m) => (
                      <a key={m.text} href={m.href} className="lsd-browse__pill">{m.text}</a>
                    ))}
  ...
  if (categoryStateRegionMode) {
    ...
    const panels = [
      { icon: "/images/Budget.png",   title: `Browse ${label} Camping Trailers by Price in ${regionName}`,            links: bandPanel(basePath, PRICE_BANDS, priceCounts) },
      { icon: "/images/ATM.png",      title: `Browse ${label} Camping Trailers by Weight (ATM) in ${regionName}`,      links: bandPanel(basePath, ATM_BANDS, atmCounts) },
      { icon: "/images/Length.png",   title: `Browse ${label} Camping Trailers by Size (Length) in ${regionName}`,     links: bandPanel(basePath, LENGTH_BANDS, lengthCounts) },
      { icon: "/images/Sleeping.png", title: `Browse ${label} Camping Trailers by Sleeping Capacity in ${regionName}`, links: bandPanel(basePath, SLEEP_BANDS, sleepCounts) },
    ];
    ...
    return (
      <section className="lsd-browse">
        <div className="container">
          ...
          {showMakePanel && (
            <>
              <h3 className="lsd-browse__panel-title">{`Browse ${label} Camping Trailers by Popular Manufacturers in ${regionName}`}</h3>
  ...
  const leftTitle  = hasState ? `Browse Camping Trailers by Region in ${stateLabel(state!)}` : "Browse Camping Trailers by State";
  const rightTitle = hasState ? `Browse Camping Trailers by Type in ${stateLabel(state!)}`   : "Browse Camping Trailers by Type";
